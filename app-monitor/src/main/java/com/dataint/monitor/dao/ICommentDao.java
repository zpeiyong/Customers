...
public interface ICommentDao extends JpaRepository<Comment, Long> {
    ...
    Integer countByArticleId(Long articleId);
      @Query(nativeQuery = true, value = "SELECT u.username as userName,com.article_id as articleId,com.created_time as createTime,com.content as content,com.user_id as userId " +
              "from comment com left JOIN `user` u " +
              "on com.user_id= u.id   where  com.article_id =?1 and  u.id =?2 ",

              countQuery="SELECT u.username as userName,com.article_id as articleId,com.created_time as createTime,com.content as content,com.user_id as userId " +
                      "from comment com left JOIN `user` u " +
                      "on com.user_id= u.id   where  com.article_id =?1 and  u.id =?2 "
      )
